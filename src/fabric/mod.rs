...
use crate::vanilla::schema::Vanilla;
use crate::{mem, util, version};
use crate::fabric::schema::{Fabric, FabricIntermediaryVersion, FabricLoaderJSON, FabricLoaderVersion, FabricVersion};
...
pub async fn down(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf) -> FabricLoaderJSON {
    ...
    let loader_jar_url = format!("{}{}", Fabric::maven(), loader.jar_path(Fabric::id()));
...
impl<'a> McLoader<'a> for schema::Fabric {
    async fn install(&mut self, ctx: McCtx<'a>) {
		...
		let loader = if let Some(ref ver_str) = ctx.opt_loader_version {
			&loader_versions
				.into_iter()
		...
		println!("Launching fabric {}-{} build {} with memory limit {} and username {}", ver.version, loader_version, loader_build, ctx.limit, ctx.username);
		...
		let ctx = McCtx { mp: ctx.mp, opt_version: Some(ver.version.clone()), limit: ctx.limit.clone(), version_dir: Some(&ver_path), username: ctx.username.clone(), javaagent: ctx.javaagent, dirs: ctx.dirs, opt_loader_version: ctx.opt_loader_version };
	...
	fn intermediary_versions() -> String {
		"https://meta.fabricmc.net/v2/versions/intermediary".to_owned()
	}
