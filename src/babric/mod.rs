use std::{fs, io::{BufRead as _, BufReader}, path::PathBuf, process::{Command, Stdio}};

use indicatif::MultiProgress;
use uuid::Uuid;

use crate::{app::{McCtx, McLaunchCtx, McLoader}, babric::schema::Babric, fabric::{get_ver, schema::{Fabric, FabricIntermediaryVersion, FabricLoaderJSON, FabricLoaderVersion, FabricVersion}}, mem, util, vanilla::schema::Vanilla, version};
...
pub async fn down_intermediary(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf) {
    ...
    let maven_path_with_domain = format!("{}{}", Babric::maven(), maven_path);
...
pub async fn down(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf) -> FabricLoaderJSON {
    ...
    let loader_jar_url = format!("{}{}", Fabric::maven(), loader.jar_path(Babric::id()));
...
impl<'a> McLoader<'a> for Babric {
    async fn install(&mut self, ctx: McCtx<'a>) {
		let game_versions = util::download_text_no_save_async(ctx.mp, &Babric::game_versions(), "Downloaded fabric game versions json".to_string()).await.expect("Failed to download fabric game versions json");
		...
		let loader_versions_json = util::download_text_no_save_async(ctx.mp, &Babric::loader_versions(), "".to_string()).await.expect("Failed to download loader versions JSON");
		...
		println!("Launching babric {}-{} build {} with memory limit {} and username {}", ver.version, loader_version, loader_build, ctx.limit, ctx.username);

		let vers = ctx.dirs.vers_dir.clone();
		let ver_path = vers.join(format!("babric-{}", ver.version.clone()));
    ...
    fn game_versions() -> String {
		"https://meta.babric.glass-launcher.net/v2/versions/game".to_string()
    }
    ...
    fn loader_versions() -> String {
		"https://meta.babric.glass-launcher.net/v2/versions/loader".to_string()
	}

    fn intermediary_versions() -> String {
		"https://meta.babric.glass-launcher.net/v2/versions/intermediary".to_string()
	}
