...
use crate::{app::{McCtx, McLaunchCtx, McLoader}, fabric::{get_ver, schema::{Fabric, FabricIntermediaryVersion, FabricLoaderJSON, FabricLoaderVersion, FabricVersion}}, mem, ornithe::schema::Ornithe, util, vanilla::schema::Vanilla, version};
use std::fs;

pub mod schema;

pub async fn down_intermediary(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf) {
	let use_release = "releases/";

    let intermediary_versions_text = util::download_text_no_save_async(
		mp,
		&Ornithe::intermediary_versions(),
    ...
    let maven_path_with_domain = format!("{}{}{}", Ornithe::maven(), use_release, maven_path);
...
pub async fn down(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf) -> FabricLoaderJSON {
    ...
    let loader_jar_url = format!("{}{}", Fabric::maven(), loader.jar_path(Ornithe::id()));
...
impl<'a> McLoader<'a> for Ornithe {
	async fn install(&mut self, ctx: McCtx<'a>) {
		...
		let ver = if ctx.opt_version.is_some() {
			get_ver(versions, ctx.opt_version.unwrap())
		} else {
		...
		println!("Launching ornithe {}-{} build {} with memory limit {} and username {}", ver.version, loader_version, loader_build, ctx.limit, ctx.username);

		let vers = ctx.dirs.vers_dir.clone();
		let ver_path = vers.join(format!("ornithe-{}", ver.version.clone()));
	...
	fn launch(&self, ctx: McLaunchCtx) {
		...
		let ver = ver_dir.file_name().unwrap().to_str().unwrap().replace("fabric-", "").replace("ornithe-", "");
		cmd.push(ver);
	...
	fn game_versions() -> String {
		"https://meta.ornithemc.net/v3/versions/gen2/game".to_string()
	}
	...
	fn loader_versions() -> String {
		"https://meta.ornithemc.net/v3/versions/gen2/fabric-loader".to_string()
	}

	fn intermediary_versions() -> String {
		"https://meta.ornithemc.net/v3/versions/gen2/intermediary".to_string()
	}

	fn maven() -> String {
		"https://maven.ornithemc.net/".to_string()
	}
}
