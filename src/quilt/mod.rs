use std::{io::{BufRead as _, BufReader}, path::{Path, PathBuf}, process::{Command, Stdio}};

use indicatif::MultiProgress;
use uuid::Uuid;

use crate::{app::{McCtx, McLaunchCtx, McLoader}, fabric::{self, schema::{Fabric, FabricIntermediaryVersion, FabricLoaderJSON, FabricLoaderVersion, FabricVersion}}, mem, quilt::schema::Quilt, util, vanilla::schema::Vanilla, version};

pub mod schema;

pub async fn down_intermediary(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf, release: bool) {
	let use_release = if release { "repository/release/" } else { "repository/snapshot/" };

    let intermediary_versions_text = util::download_text_no_save_async(
		mp,
		&Quilt::intermediary_versions(),
    ...
    let maven_path_with_domain = format!("{}{}", Fabric::maven(), maven_path);
...
pub async fn down(mp: &MultiProgress, loader: &FabricLoaderVersion, version: &FabricVersion, ver: PathBuf, release: bool) -> FabricLoaderJSON {
	let use_release = if release { "repository/release/" } else { "repository/snapshot/" };

    down_intermediary(mp, loader, version, ver.clone(), release).await;
    let loader_jar_url = format!("{}{}{}", Quilt::maven(), use_release, loader.jar_path(Quilt::id()));
    ...
    let loader_json = if !json_path.exists() {
        ...
        std::fs::read_to_string(json_path.as_path()).unwrap()
    };
    ...
    for lib in &parsed_json.libraries.common {
        ...
        let _ = std::fs::create_dir_all(lib_path.clone().parent().unwrap());
...
impl<'a> McLoader<'a> for schema::Quilt {
    async fn install(&mut self, ctx: McCtx<'a>) {
		let use_release = if self.snapshot { "repository/release/" } else { "repository/snapshot/" };

		let game_versions = util::download_text_no_save_async(
			ctx.mp,
			&Self::game_versions(),
		...
		let ver = if ctx.opt_version.is_some() {
			fabric::get_ver(versions, ctx.opt_version.unwrap())
		} else {
			versions.first().unwrap().clone()
		};

		let loader_versions_json = util::download_text_no_save_async(ctx.mp, &Self::loader_versions(), "".to_string()).await.expect("Failed to download loader versions JSON");
		...
		println!("Launching quilt {}-{} build {} with memory limit {} and username {}", ver.version, loader_version, loader_build, ctx.limit, ctx.username);

		let vers = ctx.dirs.vers_dir.clone();
		let ver_path = vers.join(format!("quilt-{}", ver.version.clone()));

		util::create_dirs(vers, ver_path.clone());

		let mut vanilla = Vanilla::default();

		let ctx = McCtx { mp: ctx.mp, opt_version: Some(ver.version.clone()), opt_loader_version: None, limit: ctx.limit, version_dir: Some(&ver_path), username: ctx.username, javaagent: ctx.javaagent, dirs: ctx.dirs };

		vanilla.install(ctx.clone()).await;


		// TODO: make it an option
		let parsed_json = down(ctx.mp, loader, &ver, ver_path.clone(), true).await;

		let _ = std::fs::remove_dir_all(ver_path.join("libs").join("META-INF"));
		let _ = std::fs::remove_dir_all(ver_path.join("libs").join("org").join("ow2").join("asm").join("asm-all"));

		let asm = ver_path.join("libs").join("org").join("ow2").join("asm").join("asm");
		let entries = std::fs::read_dir(&asm).expect("Failed to read lib dir").filter_map(|e| e.ok()).filter(|e| e.file_type().map(|ft| ft.is_dir()).unwrap_or(false));
		...
		if let Some((_, dir_name)) = min_dir {
			if len > 1 {
				let _ = std::fs::remove_dir_all(asm.join(Path::new(&dir_name)));
			}
		} else {
    ...
    fn launch(&self, ctx: McLaunchCtx) {
		...
		cmd.push(classpath);
		cmd.push(self.main_class.clone());
    ...
    fn game_versions() -> String {
		"https://meta.quiltmc.org/v3/versions/game".to_string()
    }
	...
	fn intermediary_versions() -> String {
		"https://meta.quiltmc.org/v3/versions/intermediary".to_string()
	}
